...
contract BackstopPool is Ownable, ReentrancyGuard, IBackstopPool {
    ...
    event WithdrawalCancelled(address indexed user); // New Event
    ...
    function depositLiquidity(uint256 usdcAmount) external nonReentrant {
        ...
        // Gas Optimization: Read total value once
        uint256 totalValue = liquidUsdc();
        uint256 sharesToMint = _valueToShares(usdcAmount, totalValue);
    ...
    /**
     * @notice Cancels the sender's pending withdrawal request.
     */
    function cancelWithdrawal() external nonReentrant {
        require(withdrawalRequestShares[msg.sender] > 0, "CIP: No withdrawal request to cancel");
    ...
    function withdrawLiquidity(uint256 catShareAmountBurn) external nonReentrant {
        ...
        // Gas Optimization: Read total value once
        uint256 totalValue = liquidUsdc();
        uint256 usdcToWithdraw = _sharesToValue(catShareAmountBurn, totalValue);
    ...
    function drawFund(uint256 amountToDraw) external override onlyApproved {
    ...
    function receiveProtocolAssetsForDistribution(address protocolAsset, uint256 amount) external onlyRiskManager {
        ...
        uint256 effectiveSupply = totalCatSharesSupply > INITIAL_SHARES_LOCKED
            ...
            : 0;
        rewardDistributor.distribute(CAT_POOL_REWARD_ID, protocolAsset, amount, effectiveSupply);
    ...
    function claimProtocolAssetRewardsFor(address user, address protocolAsset) external override onlyApproved {
        require(address(rewardDistributor) != address(0), "CIP: Reward distributor not set");
    ...
    function _valueToShares(uint256 usdcValue, uint256 currentTotalValueInPool) internal view returns (uint256) {
        uint256 totalCatSharesSupply = catShareToken.totalSupply();
        uint256 effectiveSupply = totalCatSharesSupply > INITIAL_SHARES_LOCKED
            ? totalCatSharesSupply - INITIAL_SHARES_LOCKED
            : 0;
    ...
    function _sharesToValue(uint256 shareAmount, uint256 currentTotalValueInPool) internal view returns (uint256) {
        uint256 totalCatSharesSupply = catShareToken.totalSupply();
        uint256 effectiveSupply = totalCatSharesSupply > INITIAL_SHARES_LOCKED
            ? totalCatSharesSupply - INITIAL_SHARES_LOCKED
            : 0;
