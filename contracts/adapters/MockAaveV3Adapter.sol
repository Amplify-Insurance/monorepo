...
/**
 * @title MockAaveV3Adapter
 * @author Gemini
 * @notice A mock yield adapter for testnet environments.
 * @dev This contract simulates the behavior of the AaveV3Adapter for testing purposes.
 * It does not interact with the actual Aave protocol. Instead, it tracks balances
 * internally and includes a helper function to simulate yield generation.
 */
contract MockAaveV3Adapter is IYieldAdapter, IYieldAdapterEmergency, Ownable, ReentrancyGuard {
    ...
    constructor(IERC20 _asset, address _initialOwner) Ownable(_initialOwner) {
        require(address(_asset) != address(0), "MockAaveV3Adapter: invalid asset");
        underlyingToken = _asset;
    }

    // Modifier to restrict calls to the CapitalPool only
    modifier onlyCapitalPool() {
        require(msg.sender == capitalPoolAddress, "MockAaveV3Adapter: Caller is not CapitalPool");
        _;
    }
    ...
    function deposit(uint256 amountToDeposit) external override onlyCapitalPool nonReentrant {
        require(amountToDeposit > 0, "MockAaveV3Adapter: amount zero");
    ...
    {
        require(to != address(0), "MockAaveV3Adapter: zero address");
    ...
    function setCapitalPoolAddress(address capitalPoolAddr) external onlyOwner {
        require(capitalPoolAddr != address(0), "MockAaveV3Adapter: Zero address");
