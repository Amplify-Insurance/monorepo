    ...
    context("With Addresses Set", function () {
        ...
        describe("Capital Deallocation", function() {
            ...
            it("Should allow an underwriter to deallocate from a pool with no losses", async function() {
                ...
                await riskManager.connect(underwriter1).requestDeallocateFromPool(POOL_ID_1, PLEDGE_AMOUNT);
            ...
            it("Should allow partial deallocation", async function () {
                const half = PLEDGE_AMOUNT / 2n;
                await mockLossDistributor.setPendingLoss(underwriter1.address, POOL_ID_1, 0);
                await riskManager.connect(underwriter1).requestDeallocateFromPool(POOL_ID_1, half);
                await expect(riskManager.connect(underwriter1).deallocateFromPool(POOL_ID_1))
                    .to.emit(riskManager, "CapitalDeallocated").withArgs(underwriter1.address, POOL_ID_1, half);
                expect(await riskManager.underwriterCapitalPerPool(underwriter1.address, POOL_ID_1)).to.equal(half);
                const allocations2 = await getAllocations(underwriter1.address);
                expect(allocations2).to.deep.equal([POOL_ID_1]);
            });
            ...
            it("Should revert if trying to deallocate from a pool they are not in", async function() {
                ...
                await expect(riskManager.connect(underwriter2).requestDeallocateFromPool(POOL_ID_1, PLEDGE_AMOUNT))
