...
export default function usePastProposals() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const query = `{ governanceProposals(orderBy: id, orderDirection: desc) { id proposer poolId pauseState votingDeadline executed passed forVotes againstVotes abstainVotes votes { id voter vote weight } } }`
        ...
        console.error('Failed to load past proposals', err)
      } finally {
        setLoading(false)
      }
    }
